#ifndef SETTINGS2XML_H
#define SETTINGS2XML_H 
...
bool readSettingsXml(QIODevice &device, QMap<QString, QVariant> &map)
{
	...
	//if it gets here, an error occurred.
	map.clear();
	return false;
}
...
bool writeSettingsXml(QIODevice &device, const QMap<QString, QVariant> &map)
{
	...
	{
		QString strippedString = unsplitKey.mid(unsplitKey.indexOf("_")+1);

		QStringList segs = strippedString.split("/", QString::SkipEmptyParts);
